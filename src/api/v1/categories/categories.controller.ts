  ...
  listCategories = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const filters: ListCategoriesFilters = {
        parentCategory:
          req.query.parentCategory === 'null' ? null : (req.query.parentCategory as string | undefined),
      };
      ...
      const { categories, total } = await this.categoryService.listCategories(filters, pagination);
